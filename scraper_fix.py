...
from helpers import copy_latest_if_page_unchanged, copy_latest_if_csv_unchanged, Filename
...
if __name__ == '__main__':
    ...
    for country in wget_websites:
        ...
        for i in range(len(ps) - 1):
            copy_latest_if_page_unchanged(ps[i], ps[i + 1], country, SoupWgets, logger)
    # selenium_websites = [k for k in WEBSITES if k in SELENIUM_DOWNLOADS]
    # for country in selenium_websites:
    #     ps = sorted([str(p) for p in Path(f'data/{country.lower()}').iterdir()])
    #     for i in range(len(ps) - 1):
    #         copy_latest_if_page_unchanged(ps[i], ps[i + 1], country, SoupSelenium, logger)
    ...
    # ps = sorted([str(p) for p in Path(f'data/{country.lower()}').iterdir() if Filename(p.name).later_than(datetime_min)])
    # for i in range(len(ps) - 1):
    #     copy_latest_if_page_unchanged(ps[i], ps[i + 1], country, SoupPosts, logger)
    ...
    # ps = sorted([str(p) for p in Path(f'data/{country.lower()}').iterdir() if Filename(p.name).later_than(datetime_min)])
    # for i in range(len(ps) - 1):
    #     copy_latest_if_csv_unchanged(ps[i], ps[i + 1], country, CSVS[country], Csv, logger)
